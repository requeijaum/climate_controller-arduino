...
{   
    ...
    if (root.success()) {*/
     ...
     //caso não tenha recebido um JSON válido
         //String bfr;
        int index,indexf,indexl;
        String temp;
        // {"t1": 12 , "pd1": "1700" }
        if ( (index = teste.indexOf("\"m\":")) >= 0){
          indexf = teste.indexOf(':', index);
          indexl = teste.indexOf(' ',indexf+2);
          temp = teste.substring(indexf+2,indexl);
          Serial.println("Achou a string entrou aqui");
          Serial.println(temp);
          root.set("m",temp.toInt());
        }
        if ( (index = teste.indexOf("\"p\":")) >= 0){
          indexf = teste.indexOf(':', index);
          indexl = teste.indexOf('"',indexf+2);
          temp = teste.substring(indexf+3,indexl);
          Serial.println("Achou a string entrou aqui");
          Serial.println(temp);
          root.set("p",temp.toInt());
        }
        if ( (index = teste.indexOf("\"pd1\":")) >= 0){
          indexf = teste.indexOf(':', index);
          indexl = teste.indexOf('"',indexf+3);
          temp = teste.substring(indexf+3,indexl);
          Serial.println("Achou a string entrou aqui");
          Serial.println(temp);
          root.set("pd1",temp);
        }
        if ( (index = teste.indexOf("\"pd2\":")) >= 0){
          indexf = teste.indexOf(':', index);
          indexl = teste.indexOf('"',indexf+3);
          temp = teste.substring(indexf+3,indexl);
          Serial.println("Achou a string entrou aqui");
        ...
        if ( (index = teste.indexOf("\"t1\":")) >= 0){
          indexf = teste.indexOf(':', index);
          indexl = teste.indexOf(' ',indexf+2);
          temp = teste.substring(indexf+2,indexl);
          Serial.println("Achou a string entrou aqui");
          Serial.println(temp);
          root.set("t1",temp.toInt());
        }
        if ( (index = teste.indexOf("\"t2\":")) >= 0){
          indexf = teste.indexOf(':', index);
          indexl = teste.indexOf(' ',indexf+2);
          temp = teste.substring(indexf+2,indexl);
          Serial.println("Achou a string entrou aqui");
          Serial.println(temp);
          root.set("t2",temp.toInt());
        }
        if ( (index = teste.indexOf("\"tt\":")) >= 0){
          indexf = teste.indexOf(':', index);
          indexl = teste.indexOf(' ',indexf+2);
          temp = teste.substring(indexf+2,indexl);
          Serial.println("Achou a string entrou aqui");
          Serial.println(temp);
          root.set("tt",temp.toInt());
        }
        /*
...
void serialize(char* json, size_t maxSize, char* copy)
{
...
// Timer settings
#define START_TIME  700//prog.pl1
#define END_TIME    1800//prog.pd1
...
#define TEMPERATURA_IDEAL   23.0//data.tMax    //era 23.0
#define TEMP_MIN            22.5//data.tMin    //era 22.5  //só posso usar int no json
...
//------------------------
    ...
    serialize(teste, JSON_OUT_SIZE, copia);
